...
- **Android >= 4.4** : [une image de Chrome pour Android](http://developer.android.com/about/versions/kitkat.html#44-webview), mais qui, a priori, se met à jour uniquement avec Android lui-même 
...
Voici un extrait d'un [rapport de bug Android](https://code.google.com/p/chromium/issues/detail?id=113088) qui résume bien la problématique : 
...
Voyons donc les [travaux proposées](https://code.google.com/p/chromium/issues/detail?id=113088#c86) qui me paraissent les plus prometteurs :

- Chromium/Chrome en tant que nouvelle *webview*, soit en tant que [projet Google](http://code.google.com/p/chromium/issues/detail?id=113088#c100) ou en tant que [projet "communautaire"](https://github.com/davisford/chromeview)
- Le projet [Geckoview](https://wiki.mozilla.org/Mobile/GeckoView) de Mozilla

{% pullquote %}
C'est surtout le deuxième qui me semble le plus prometteur, parce qu'il ne se restreint pas à la seule plateforme Android. {" L'ambition de ce projet est de fournir une webview basée sur Firefox pour toutes les plateformes, d'aujourd'hui ou à venir, mobiles ou non "} : Android, Windows, Linux, ... Seul [iOS fait exception](https://support.mozilla.org/fr/kb/firefox-disponible-pour-iphone-ipad) car les restrictions imposées par Apple ne permettent pas de déployer Firefox dans de bonnes conditions. Mais c'est sans conséquence pour une application HTML5, iOS étant une plateforme de choix pour exécuter du HTML5 comme nous l'avons déjà évoqué plus haut. Au prix de 25Mo (!), Geckoview est compatible avec les versions Android 2.x / 3.x / 4.x et avec toutes les fonctionnalités HTML5 avancées : à vous SSE, websockets, WebGL, ...
{% endpullquote %}

Le projet est très actif et il est d'ores et déjà possible de monter des prototypes avec [le code mis à disposition par l'équipe de développement](https://wiki.mozilla.org/Mobile/GeckoView), qui propose notamment [un navigateur basé sur Geckoview](https://github.com/mfinkle/geckobrowser). Pour l'avoir déjà mis en oeuvre sur un projet à titre de démonstration, c'est très, très prometteur : sur la fameuse application qui veut se faire passer pour du natif dont je vous ai parlé plus haut, les performances du HTML5 sont très proches de celles du natif !
